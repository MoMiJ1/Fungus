...
class GA_extremum:
    ...
    def __init__(self, M_Size, T, Pc, Pm, bound_min, bound_max):
        self.M_Size = M_Size         # 群体数量
        self.T = T                   # 遗传代数
        self.Pc = Pc                 # 变异概率
        self.Pm = Pm                 # 交叉概率
        self.bound_min = bound_min   # 搜索上界
        self.bound_max = bound_max   # 搜索下界
        self.Optimal = [0, 0]        # 最优解 [x , max(func)]
    ...
    def mutation(self, M, t):
        if t < 0.8 * self.T:
            ...
            for i in range(self.M_Size):
                if np.random.random() < self.Pm:
                    M[i] = sorted([M[i] + np.random.choice([-1,1]) * random.gauss((self.bound_min + self.bound_max) / 2, 2 * (self.bound_max - self.bound_min) / self.M_Size), 0, 1])[1]
        return M
